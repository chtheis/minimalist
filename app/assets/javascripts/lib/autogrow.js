  ...
  $.fn.autogrow = function(options) {
    ...
    function resize(e) {
      ...
      if (contentLength != $e.data("autogrow-length") || elementWidth != $e.data("autogrow-width")) {
        ...
        // added by mshwery
        var paddingTop = $e.css('padding-top').replace('px','')
        var paddingBottom = $e.css('padding-bottom').replace('px','')

        // subtractions are by mshwery
        var height = Math.max($e.data("autogrow-min"), Math.ceil(Math.min(
          $e.prop("scrollHeight") - paddingTop - paddingBottom + options.expandTolerance * $e.data("autogrow-line-height"), 
          $e.data("autogrow-max"))));
    ...
    this.each(function() {
      ...
      if (!$this.data("autogrow-initialized")) {
        //$this.css("padding-top", 0).css("padding-bottom", 0); removed by mshwery
        $this.bind("keydown", resize).bind("focus", resize);
